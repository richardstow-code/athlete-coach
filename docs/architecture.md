...
**v1.2.3 — action dangle (post-build, canary id=367 under v1.2.2).** The only residual: `verdict.action` overran its cap as a semicolon-joined two-part instruction and dangled on a content word ("…to settle into a true") that v1.2.2's comma-only fallback didn't catch. Fix: (a) prompt makes `action` ONE short complete next-step (cap **110**, no `;`-joined run-on, no metric dump); (b) `clampText` fallback is now ordered — last sentence terminator → last **clause punctuation (`, ; :`)** dropping the trailing partial clause → word boundary with trailing dangling-word + "determiner + content-word" (e.g. "a true") strip — and it **guarantees a terminal `.`** on the fallback paths. `SCHEMA_VERSION` → `analyze-activity@v1.2.3`.
