...
export default function PostWorkoutPopup({ onDismiss, onViewDetail }) {
  ...
  const [enrichStatus, setEnrichStatus] = useState(null) // 'pending'|'processing'|'complete'|'failed'

  // Check-in state
  const [sessionFeel, setSessionFeel] = useState(null)
  const [injuryChoice, setInjuryChoice] = useState(null)
  ...
  useEffect(() => {
    async function checkLatest() {
      ...
      setActivity(latest)
      setEnrichStatus(latest.enrichment_status || 'complete')
      setVisible(true)

      if (latest.enrichment_status === 'complete') {
        // Already enriched — load feedback immediately
        await loadFeedback(latest)
      }
    }
  ...
  // ── Polling when enrichment is in progress ────────────────

  useEffect(() => {
    if (!activity || !visible) return

    const isWaiting = enrichStatus === 'pending' || enrichStatus === 'processing'
    if (!isWaiting) {
      stopPolling()
      return
    }
    ...
    return stopPolling
  }, [activity, enrichStatus, visible])

  function startPolling(act) {
    stopPolling()
    ...
    timeoutRef.current = setTimeout(() => {
      stopPolling()
      setEnrichStatus(prev => (prev === 'pending' || prev === 'processing') ? 'failed' : prev)
    }, POLL_TIMEOUT_MS)
  }
  ...
  async function loadFeedback(act) {
    const { data: mem } = await supabase
      .from('coaching_memory')
      .select('content, created_at')
      .eq('activity_id', parseInt(String(act.strava_id), 10))
  ...
  function renderBasicStats() {
    const pace = activity.distance_km && activity.duration_min
      ? (() => {
          const s = Math.round((activity.duration_min * 60) / parseFloat(activity.distance_km))
    ...
    return (
      <>
        ...
        <div style={{ textAlign: 'center', padding: '24px 0 32px' }}>
          ...
          <div style={{ fontSize: 13, color: Z.text, marginBottom: 6 }}>Analysing your run…</div>
          <div style={{ fontSize: 11, color: Z.muted }}>Full coaching feedback incoming</div>
          <style>{`@keyframes pulse { 0%,100%{opacity:0.2;transform:scale(0.85)} 50%{opacity:1;transform:scale(1)} }`}</style>
        </div>
      </>
    )
  }
  ...
  function renderCompleteState() {
    ...
    const totalZoneSecs = Object.values(zones).reduce((s, v) => s + (Number(v) || 0), 0)

    return (
      <>
        {renderBasicStats()}

        {/* Zone bars from zone_data */}
        {totalZoneSecs > 0 && (
          <div style={{ marginBottom: 20 }}>
            ...
            {[
              ...
              { z: 'z5', label: 'Z5 Max',       color: '#ff2222' },
            ].map(({ z, label, color }) => {
              const pct = zones[z] ? Math.round((Number(zones[z]) / totalZoneSecs) * 100) : 0
        ...
        {/* Check-in */}
        {!checkinResult && (
          <div style={{ background: Z.surface, border: `1px solid ${Z.border}`, borderRadius: 12, padding: 16, marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: injuryChoice === 'flag' ? 12 : 0 }}>
              <button
                ...
                style={{ flex: 1, padding: 9, borderRadius: 8, cursor: 'pointer', background: injuryChoice === 'good' ? 'rgba(77,255,145,0.12)' : 'rgba(255,255,255,0.04)', border: injuryChoice === 'good' ? `1px solid ${Z.green}` : `1px solid ${Z.border}`, color: injuryChoice === 'good' ? Z.green : Z.muted, fontFamily: "'DM Mono', monospace", fontSize: 12 }}>
                All good 👍
              </button>
              <button
                onClick={() => setInjuryChoice(injuryChoice === 'flag' ? null : 'flag')}
                style={{ flex: 1, padding: 9, borderRadius: 8, cursor: 'pointer', background: injuryChoice === 'flag' ? 'rgba(255,179,71,0.12)' : 'rgba(255,255,255,0.04)', border: injuryChoice === 'flag' ? `1px solid ${Z.amber}` : `1px solid ${Z.border}`, color: injuryChoice === 'flag' ? Z.amber : Z.muted, fontFamily: "'DM Mono', monospace", fontSize: 12 }}>
                Flag something ⚠️
              </button>
            </div>
            {injuryChoice === 'flag' && (
              <div style={{ marginTop: 12 }}>
                <input
                  ...
                  style={{ width: '100%', boxSizing: 'border-box', background: '#161616', border: `1px solid ${Z.border2}`, borderRadius: 8, padding: '10px 12px', marginBottom: 8, color: Z.text, fontSize: 12, fontFamily: "'DM Mono', monospace", outline: 'none' }}
                />
                <textarea
                  value={injuryNotes}
                  ...
                  rows={3}
                  style={{ width: '100%', boxSizing: 'border-box', background: '#161616', border: `1px solid ${Z.border2}`, borderRadius: 8, padding: '10px 12px', color: Z.text, fontSize: 12, fontFamily: "'DM Mono', monospace", resize: 'none', outline: 'none', lineHeight: 1.5 }}
                />
              </div>
            )}
          </div>
        ...
        {!checkinResult && (
          <div style={{ display: 'flex', gap: 10 }}>
            <button
              ...
              disabled={saving}
              style={{ flex: 1, background: saving ? '#1a1a1a' : Z.accent, border: 'none', borderRadius: 8, padding: 12, fontFamily: "'DM Mono', monospace", fontSize: 13, cursor: saving ? 'wait' : 'pointer', color: saving ? Z.muted : Z.bg, fontWeight: 500 }}
            >
              {saving ? 'Saving...' : 'Save check-in'}
            </button>
            <button
              onClick={viewDetail}
              style={{ flex: 1, background: 'none', border: `1px solid ${Z.border2}`, borderRadius: 8, padding: 12, fontFamily: "'DM Mono', monospace", fontSize: 13, cursor: 'pointer', color: Z.muted }}
            >
              Full breakdown →
            </button>
          </div>
  ...
  function renderErrorState() {
    return (
      <>
        {renderBasicStats()}
        <div style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${Z.border}`, borderRadius: 10, padding: '16px 14px', marginBottom: 20, textAlign: 'center' }}>
          <div style={{ fontSize: 13, color: Z.muted, marginBottom: 6 }}>Analysis took longer than expected.</div>
          <div style={{ fontSize: 11, color: Z.muted, lineHeight: 1.6 }}>Your activity has been saved. Check back in the app shortly.</div>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button
            onClick={viewDetail}
            style={{ flex: 1, background: Z.accent, border: 'none', borderRadius: 8, padding: 12, fontFamily: "'DM Mono', monospace", fontSize: 13, cursor: 'pointer', color: Z.bg, fontWeight: 500 }}
          >
            View activity →
          </button>
          <button
  ...
  const isLoading = enrichStatus === 'pending' || enrichStatus === 'processing'
  const isFailed  = enrichStatus === 'failed'
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 300, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      ...
      <div style={{ position: 'relative', background: Z.bg, borderRadius: '16px 16px 0 0', border: `1px solid ${Z.border2}`, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ padding: '16px 20px 36px' }}>
          ...
          <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 24, fontWeight: 800, color: Z.text, marginBottom: 4 }}>
            {activity.name}
          </div>
          <div style={{ fontSize: 12, color: Z.muted, marginBottom: 20 }}>
            {new Date((activity.date?.slice(0, 10) || '') + 'T12:00:00').toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' })}
          </div>

          {isLoading && renderLoadingState()}
          {isFailed  && renderErrorState()}
          {!isLoading && !isFailed && renderCompleteState()}
        </div>
      </div>
    </div>
  )
